...
QtSoapType::QtSoapType(const QtSoapType &copy)
    : t(copy.t), errorStr(copy.errorStr), i(copy.i),
      n(copy.n), u(copy.u), h(copy.h), attrs(copy.attrs)
{
}
...
/*!
    ...
    nothing.
    DM: Added functionality to empty attribute list.
*/
void QtSoapType::clear()
{
    //Remove existing attributes
    //qDebug() << "QtSoapType::clear: Attempting to clear n attrs: " << attrs.length();
    for (unsigned int atNum=attrs.length(); atNum ; --atNum) {
	attrs.removeNamedItem(attrs.item(atNum-1).toAttr().name());
    }
}
...
//DM: Returns the attributes
QDomNamedNodeMap QtSoapType::attributes() const
{
    return attrs;
}
...
void QtSoapType::setAttribute(const QString &name, const QString &value)
{
    //qDebug() << "QtSoapType::setAttribute: Trying to set attribute: " << name;

    QDomDocument doc("tempdoc");
    QDomElement root = doc.createElement("tempelement");
    doc.appendChild(root);
    root.setAttribute(name, value);

    //Append existing attributes
    for (unsigned int atNum=0; atNum<attrs.length(); atNum++) {
	QDomAttr newAttr = attrs.item(atNum).toAttr();
	root.setAttributeNode(newAttr);
    }

    this->attrs = root.attributes();

    //qDebug() << "QtSoapType::setAttribute: Now have n attrs: " << attrs.length();
}

QString QtSoapType::getAttribute(const QString &name)
{
...
void QtSoapStruct::clear()
{
    dict.clear();
    //Remove existing attributes
    //qDebug() << "QtSoapStruct::clear: Attempting to clear n attrs: " << attrs.length();
    for (unsigned int atNum=attrs.length(); atNum ; --atNum) {
	attrs.removeNamedItem(attrs.item(atNum-1).toAttr().name());
    }
}
...
QDomElement QtSoapStruct::toDomElement(QDomDocument doc) const
{
    ...
    //qDebug() << "QtSoapStruct::toDomElement: Have n attributes: " << attrs.length();
    for (unsigned int atNum=0; atNum<attrs.length(); atNum++) {
	//qDebug() << "QtSoapStruct::toDomElement: Setting attribute: " << attrs.item(atNum).toAttr().name();
	QDomAttr newAttr = attrs.item(atNum).toAttr();
...
bool QtSoapStruct::parse(QDomNode node)
{
	...
	qDebug() << "QtSoapStruct::parse:" << n.toElement().tagName() 
		 << " element has n attributes: " << attrs.length();
	for (unsigned int atNum=0; atNum<attrs.length(); atNum++) {
	    QDomAttr a = attrs.item(atNum).toAttr();
...
void QtSoapSimpleType::clear()
{
    v.clear();
    //Remove existing attributes
    //qDebug() << "QtSoapSimpleType::clear: Attempting to clear n attrs: " << attrs.length();
    for (unsigned int atNum=attrs.length(); atNum ; --atNum) {
	attrs.removeNamedItem(attrs.item(atNum-1).toAttr().name());
    }
}
...
QDomElement QtSoapSimpleType::toDomElement(QDomDocument doc) const
{
    ...
    //qDebug() << "QtSoapSimpleType::toDomElement: DM Preventing type and namespace attributes from being added";
    /*
    QString schemaprefix = QtSoapNamespaces::instance().prefixFor(XML_SCHEMA_INSTANCE);
    a.setAttributeNS(XML_SCHEMA_INSTANCE, schemaprefix + ":type", "xsd:" + typeName());
    */

    //qDebug() << "QtSoapSimpleType::toDomElement: Have n attributes: " << attrs.length();
    for (unsigned int atNum=0; atNum<attrs.length(); atNum++) {
	//qDebug() << "QtSoapSimpleType::toDomElement: Setting attribute: " << attrs.item(atNum).toAttr().name();
	QDomAttr newAttr = attrs.item(atNum).toAttr();
	a.setAttributeNode(newAttr);
    }
...
bool QtSoapSimpleType::parse(QDomNode node)
{
    ...
    attrs = node.attributes();
    //qDebug() << "QtSoapSimpleType::parse: attrs has n attributes: " << attrs.length();
...
QString QtSoapMessage::toXmlString(int indent) const
{
    ...
    env.setAttribute("xmlns:" + QtSoapNamespaces::instance().prefixFor(SOAPv11_ENCODING),
		     SOAPv11_ENCODING);
...
const QtSoapType &QtSoapMessage::returnValue() const
{
    ...
    if (!meth.isValid()) {
	qDebug() << "QtSoapMessage::returnValue: invalid method";
	return NIL;
    }

    if (meth.type() == QtSoapType::Struct) {
	...
	return *mi.data();
        ...
        //qDebug() << "QtSoapMessage::returnValue: returnValue is not QtSoapStruct, so returning body";
	return (QtSoapType &)body();
    }
}
...
void QtSoapMessage::addMethodAttribute(const QString &name, const QString &value)
{
    if (body().count() == 0) {
	qWarning("Attempted to add attribute (%s:%s) without first setting method",
...
QtSmartPtr<QtSoapType> QtSoapTypeFactory::soapType(QDomNode node) const
{
    ...
    if (!attr.isNull()) {
    	//DM: checking attribute
	//qDebug() << "QtSoapTypeFactory::soapType: " << attr.name() << "=>" << attr.value();
...
void QtSoapHttpTransport::submitRequest(QtSoapMessage &request, const QString &path)
{
    ...
    //DM: Changed to HTTP/1.1 to work with Juniper
    header.setRequest("POST", path, 1, 1);
...
void QtSoapHttpTransport::ignoreSslErrors()
{
    http.ignoreSslErrors();
}

QHttpResponseHeader QtSoapHttpTransport::lastResponse()
{
    return http.lastResponse();
}

QHttpRequestHeader QtSoapHttpTransport::currentRequest()
{
    return http.currentRequest();
}

void QtSoapHttpTransport::setSocket(QTcpSocket *socket)
{
    http.setSocket(socket);
}

void QtSoapHttpTransport::setProxy(const QNetworkProxy &proxy)
{
    http.setProxy(proxy);
}

void QtSoapHttpTransport::setKeepAlive()
{
    header.setValue("Connection","keep-alive");
}

void QtSoapHttpTransport::setUser(const QString & userName, const QString & password)
{
    qDebug() << "Setting http user and password: " << userName << password;
    http.setUser(userName, password);
    QString up = userName + ":" + password;
    QByteArray up64 = QByteArray(up.toAscii()).toBase64();
    header.setValue("Authorization", QString("Basic " + up64));
}
